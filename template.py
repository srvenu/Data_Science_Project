...
def update_friends_list(friends_list):
    st.session_state['Friends'] = friends_list

def tile_item(column, item):
    with column:
        st.button('📖', key=str(uuid.uuid4()), on_click=select_book, args=(item['ISBN'], ))
...
def recommendations(df):
    ...
    if nbr_items != 0:
        ...
        # Apply tile_item to each column-item tuple
...
def welcome_user():
    ...
    st.sidebar.write('Start reading books to get more personalized recommendations 🧐')
...
def friend_not_found():
    ...
    st.sidebar.write('Please insert only one User-ID to add to your friend list.')
